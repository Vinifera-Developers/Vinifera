...
function(GetGitState _working_dir)
	...
	# CCHyper (26/12/2020)
	# Split the time, date and time-zone into separate strings.
    string(REPLACE " " ";" TEMP_DATE_VAR "$ENV{GIT_COMMIT_DATE_ISO8601}")
    list(LENGTH TEMP_DATE_VAR TEMP_DATE_VAR_COUNT)
    if(TEMP_DATE_VAR_COUNT GREATER 0)
        list(GET TEMP_DATE_VAR 0 GIT_TMP_DATE)
    else()
        set(GIT_TMP_DATE "")
    endif()
    if(TEMP_DATE_VAR_COUNT GREATER 1)
        list(GET TEMP_DATE_VAR 1 GIT_TMP_TIME)
    else()
        set(GIT_TMP_TIME "")
    endif()
    if(TEMP_DATE_VAR_COUNT GREATER 2)
        list(GET TEMP_DATE_VAR 2 GIT_TMP_TZONE)
    else()
        set(GIT_TMP_TZONE "")
    endif()
	set(ENV{GIT_COMMIT_DATE} "${GIT_TMP_DATE}")
	set(ENV{GIT_COMMIT_TIME} "${GIT_TMP_TIME}")
	set(ENV{GIT_COMMIT_TZONE} "${GIT_TMP_TZONE}")
	...
	execute_process(
		...
		OUTPUT_VARIABLE GIT_TMP_BRANCH
		OUTPUT_STRIP_TRAILING_WHITESPACE
		ERROR_QUIET
	)
	set(ENV{GIT_BRANCH} ${GIT_TMP_BRANCH})
    ...
    execute_process(
        ...
        WORKING_DIRECTORY ${CMAKE_SOURCE_DIR}
        OUTPUT_VARIABLE GIT_TMP_COMMIT_COUNT
        OUTPUT_STRIP_TRAILING_WHITESPACE
        ERROR_QUIET
    )
	set(ENV{GIT_COMMIT_COUNT} ${GIT_TMP_COMMIT_COUNT})
...
# Description: Create and populate the initial gitinfo, before build is executed.
# Author: CCHyper (26/12/2020)
function(GitWatcher_SetupInitial)
	CheckGit("${GIT_WORKING_DIR}" changed)
	if(EXISTS "${GIT_POST_CONFIGURE_FILE}")
		file(REMOVE ${GIT_POST_CONFIGURE_FILE})
	endif()
	GitStateChangedAction()
endfunction()
