...
This section collects the audio-related controls that are otherwise spread across several pages.

Vinifera replaces the legacy DirectSound backend with MiniAudio, but keeps the game's logical naming model. In practice, existing `VocType`, `VoxType`, and `ThemeType` references still point at logical names such as `GATEUP`, `00-I150`, or `MAPS`, while the engine now resolves those names against multiple audio formats.
...
- Supported formats are searched in this order: `FLAC`, `WAV`, `OGG`, `MP3`, then `AUD`. The first file found wins.
- You therefore do not put file extensions in `SOUND.INI`, `THEME.INI`, `SPEECH.INI`, `RULES.INI`, or `ART.INI` references. If both `MAPS.FLAC` and `MAPS.AUD` exist, `MAPS.FLAC` will be used.
- Firestorm companion control files load additively instead of replacing the base database: `SOUND01.INI` after `SOUND.INI`, `THEME01.INI` after `THEME.INI`, and `SPEECH01.INI` after `SPEECH.INI`.
- Later files may override an existing entry by repeating its logical name in the relevant list section and redefining only the keys you want to change.
- Volume groups are split as follows:
  - Score volume controls music and ambient tracks.
  - Voice volume controls EVA / speech.
  - Sound volume controls gameplay SFX, UI sounds, and event loops.

```{note}
Because lookup is basename-based, a classic mod can often replace `*.AUD` with `*.FLAC`, `*.OGG`, `*.MP3`, or `*.WAV` under the same logical name without changing any INI references.
```

### Sound Effects

`SOUND.INI` defines the `VocType` database used by the game's sound effects. `SOUND01.INI` works the same way and is layered on top when Firestorm is installed.
...
- `[Defaults]->Limit`: global simultaneous-instance limit used for newly created sound entries.
- `[Defaults]->Range`: global audible range, in cells, used for newly created sound entries.
- `[Defaults]->Priority`: legacy priority number forwarded into the new concurrency system.
- `[Defaults]->Volume`: default per-entry volume multiplier in the `0.0` to `1.0` range.
- `[Defaults]->MinVolume`: default minimum volume floor used by `GLOBAL` sounds.
- `[SoundList]`: list of logical sound names. Repeating an existing name in `SOUND01.INI` updates that sound entry instead of creating a separate one.
- `[SOMESOUND]->Sounds`: comma-separated list of alternate basenames for the sound entry. Do not include extensions.
- `[SOMESOUND]->Limit`: maximum number of simultaneous instances of this sound entry. If omitted, inherits `[Defaults]->Limit`.
- `[SOMESOUND]->LoopLimit`: total number of plays for a `Control=LOOP` sound. `1` plays once, `N` plays exactly `N` times, and `0` or omission means unlimited looping.
- `[SOMESOUND]->Range`: audible range of this sound entry, in cells, for positional attenuation. If omitted, inherits `[Defaults]->Range`.
- `[SOMESOUND]->Priority`: per-entry priority used when too many copies of the same sample want to play at once.
- `[SOMESOUND]->Volume`: per-entry volume multiplier in the `0.0` to `1.0` range.
- `[SOMESOUND]->MinVolume`: minimum audible floor for `GLOBAL` sounds.
- `[SOMESOUND]->VShift`: random additional volume modifier, in hundredths. For example, `0,5` adds a random `0.00` to `0.05`.
- `[SOMESOUND]->FShift`: random pitch offset, in percent. For example, `-5,5` produces a pitch between `0.95` and `1.05`.
- `[SOMESOUND]->Type`: comma-separated sound-type flags. The current loader recognizes `NORMAL`, `GLOBAL`, `LOCAL`, `UNSHROUDED`, and `SHROUDED`.
- `GLOBAL`: never falls below `MinVolume` when positional attenuation would otherwise mute it.
- `LOCAL`: uses local positional falloff instead of the default screen-biased falloff.
- `UNSHROUDED`: only audible if the source cell is currently visible or fog-visible to the player.
- `SHROUDED`: only audible if the source cell is currently hidden from the player.
- Omitting `Type=` keeps the default screen-based positional behavior.
- `[SOMESOUND]->Control`: comma-separated control flags. The current loader recognizes `LOOP`, `RANDOM`, `PREDELAY`, `QUEUE`, and `AMBIENT`.
- `LOOP`: loops the sound indefinitely unless `LoopLimit` caps the total number of plays.
- `QUEUE`: if the sound's concurrent limit is already full, the new request waits for the next free slot instead of being dropped.
- `AMBIENT`: compatibility/classification flag only. Use `LOOP` when a voc must actually keep playing.

```{note}
Only the `Type=` and `Control=` tokens listed above are parsed by the current backend. This section intentionally documents the keys the current code actually reads, not every historical Westwood comment.
```

### Themes

`THEME.INI` defines the music database used by the jukebox, menu tracks, and other theme playback. `THEME01.INI` is additive in the same way as `SOUND01.INI`.
...
- `[General]->FadeOutSeconds`: fade-out duration used when a theme is explicitly stopped with fading.
- `[General]->CrossFading`: enables cross-faded theme transitions.
- `[General]->CrossFadeSeconds`: fade duration used when cross-fading between tracks.
- `[Themes]`: ordered list of logical theme names. Repeating a theme name in `THEME01.INI` updates it in place.
- `[SOMETHEME]->Sound`: actual file basename to play. Omit it to use the theme name itself as the filename.
- `[SOMETHEME]->Name`: full display name shown by the jukebox and other theme UI.
- `[SOMETHEME]->Artist`: display-only artist metadata.
- `[SOMETHEME]->Length`: track length in seconds.
- `[SOMETHEME]->Volume`: per-track volume multiplier in the `0.0` to `1.0` range.
- `[SOMETHEME]->Normal`: whether the track participates in the normal in-game rotation. Set this to `no` for special-purpose tracks such as menu or scripted music.
- `[SOMETHEME]->Repeat`: forces this track to requeue itself even if the global repeat option is off.
- `[SOMETHEME]->Scenario`: earliest single-player scenario in which the track is allowed to appear.
- `[SOMETHEME]->Side`: comma-separated list of side names allowed to use the track.
- `[SOMETHEME]->RequiredAddon`: addon gate for the track. Currently `0` means base game and `1` means Firestorm.

```{note}
If a theme named `DSHPLOAD` exists, the Dropship Loadout screen will use it. Otherwise Vinifera falls back to the `MAPS` theme for that screen.
```

```{note}
Vinifera also exposes jukebox commands named `PrevTheme` and `NextTheme`, shown in the UI as `Music: Previous Track` and `Music: Next Track`.
```

### EVA Speech

`SPEECH.INI` overrides EVA and other speech entries. Unlike `SOUND.INI` and `THEME.INI`, the vanilla speech table is seeded internally first, so `SPEECH.INI` is optional and only needed for overrides or additions. `SPEECH01.INI` is additive.
...
- `[Defaults]->Priority`: default speech priority.
- `[Defaults]->Delay`: global delay, in seconds, before a speech line starts playing.
- `[Defaults]->Volume`: default speech volume multiplier.
- `[Defaults]->MinVolume`: default minimum speech volume.
- `[DialogList]`: list of speech entries to update or add.
- `[SOMEVOX]->Sound`: filename override for the speech entry, without extension.
- `[SOMEVOX]->Text`: debug/description text only. It is not spoken.
- `[SOMEVOX]->Priority`: per-entry speech priority.
- `[SOMEVOX]->Volume`: per-entry speech volume multiplier.
- `[SOMEVOX]->FShift`: per-entry playback pitch multiplier.

```{note}
Speech entries that use the classic `00-Ixxx` naming pattern automatically try a side-specific `xx-Ixxx` file for the active speech side if such a file exists. This lets you provide per-side EVA variants without changing the logical `VoxType`.
```

```{note}
To override one built-in speech line, include that logical name in `[DialogList]` and then define only the keys you want to replace in its section.
```

### Gameplay Sound Hooks

The following gameplay-facing keys are not part of the audio control INIs themselves, but they hook directly into the new audio system.
...
[SOMETECHNO_OR_BUILDING]
AmbientSound=

[SOMESUPERWEAPON]
MissileLaunchedVoice=00-I150
```
...
- `[AudioVisual]->UpgradeVeteranSound`: sound effect played when a unit promotes to veteran.
- `[AudioVisual]->UpgradeEliteSound`: sound effect played when a unit promotes to elite.
- `[AudioVisual]->VoxUnitPromoted`: EVA line played on promotion.
- `[AudioVisual]->EliteFlashTimer`: how long newly elite units flash.
- `[AudioVisual]->PlaceBeaconSound`: sound effect when the player places a beacon.
- `[AudioVisual]->PlaceBeaconVoice`: EVA line when the player places a beacon.
- `[AudioVisual]->DetectBeaconVoice`: EVA line when another player places a beacon.
- `[AudioVisual]->IronCurtainSound`: sound effect played when Iron Curtain is applied.
- `[SOMETECHNO]->EnterTransportSound`: sound effect played by the transport when a passenger enters it.
- `[SOMETECHNO]->LeaveTransportSound`: sound effect played by the transport when a passenger leaves it.
- `[SOMETECHNO]->VoiceCapture`: list of response voices used when the object is ordered to capture.
- `[SOMETECHNO]->VoiceEnter`: list of response voices used when the object is ordered to enter.
- `[SOMETECHNO]->VoiceDeploy`: list of response voices used when the object is ordered to deploy / unload.
- `[SOMETECHNO]->VoiceHarvest`: list of response voices used when the object is ordered to harvest.
- If a `VoiceCapture`, `VoiceEnter`, `VoiceDeploy`, or `VoiceHarvest` list is not provided, Vinifera falls back to the object's normal `VoiceMove` responses.
- `[SOMETECHNO]->CloakSound`: per-type cloaking sound override. If omitted, the game falls back to the global cloak sound.
- `[SOMETECHNO]->UncloakSound`: per-type uncloaking sound override. If omitted, the game also falls back to the global cloak sound.
- `[SOMEBUILDING]->GateUpSound`: per-building gate-opening sound override. If omitted, the global gate-opening sound is used.
- `[SOMEBUILDING]->GateDownSound`: per-building gate-closing sound override. If omitted, the global gate-closing sound is used.
- `[SOMEOBJECT]->AmbientSound`: the name of a voc (from `SOUND.INI`) that plays continuously at the object's position while it is deployed. The voc must be flagged `Control=LOOP` in `SOUND.INI` for looping playback; `Control=AMBIENT` is optional metadata only. Positional volume/panning follow the normal voc `Range`/`Volume`/`Type` rules. Stops when the object enters limbo or is destroyed.
- `[SOMESUPERWEAPON]->MissileLaunchedVoice`: EVA line used by `MultiMissile` and `ChemMissile` super weapons when they launch.
- `[SOMEANIM]->StopSound`: sound played when the animation finishes or is removed.
- `[SOMEVOXELANIM]->StopSound`: sound played when the voxel animation finishes or is removed.

### Current Notes

- Ambient sounds (both the Ion Storm track and per-object `AmbientSound=` events) are managed by the unified voc layer. The `IONSTORM` voc, and any voc referenced via `AmbientSound=` on an object type, should be flagged `Control=LOOP` in `SOUND.INI` if they need to loop indefinitely. `Control=AMBIENT` remains optional classification only. Ion Storm ambient remains non-positional; per-object ambient sounds follow the voc's `Type` / `Range` rules for positional attenuation.
- `SPEECH.INI` supports per-entry overrides for the keys listed above, but the dependable way to set speech start delay today is still `[Defaults]->Delay`.
- This section intentionally focuses on the controls the current audio backend actually consumes. If you are migrating a legacy `SOUND.INI`, `THEME.INI`, or `SPEECH.INI`, prefer these documented keys first.
