...
bool AudioInstanceClass::Load()
{
    ...
    if (Audio_IsAUDFile(Template->Get_FileName())) {
        ...
        /*
         *  Ask ma_decoder to output f32, matching miniaudio's native mixer
         *  format. If the AUD source is exposed as s16, miniaudio's sound node
         *  can fill an internal conversion cache, observe EOF on a follow-up
         *  read, and stop before processing a very short final chunk.
         */
