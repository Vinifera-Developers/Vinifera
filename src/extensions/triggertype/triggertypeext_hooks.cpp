...
DECLARE_PATCH(_TriggerTypeClass_Read_INI_Load_Difficulty_Patch)
{
    ...
    //DEBUG_INFO("TriggerTypeClass::Read_INI() - IniName: %s Diffs: %d,%d,%d\n",
    //    this_ptr->IniName, this_ptr->Easy, this_ptr->Normal, this_ptr->Hard);
...
void TriggerTypeClassExtension_Hooks()
{
     ...
     */
    Patch_Dword(0x0064A2DC, 0x90909090); // 4 nops
    Patch_Dword(0x0064A2E9, 0x90909090); // 4 nops
    Patch_Dword(0x0064A2ED, 0x90654588); // mov [ebp+65h], al; nop;

    Patch_Dword(0x0064A2FF, 0x90909090); // 4 nops
    Patch_Dword(0x0064A30C, 0x90909090); // 4 nops
    Patch_Dword(0x0064A310, 0x90664588); // mov [ebp+66h], al; nop;

    Patch_Dword(0x0064A322, 0x90909090); // 4 nops
    Patch_Dword(0x0064A32F, 0x90909090); // 4 nops
