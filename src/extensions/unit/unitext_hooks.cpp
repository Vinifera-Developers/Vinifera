...
DECLARE_PATCH(_UnitClass_Draw_Voxel_Water_Image_Patch)
{
    ...
    unitext = UnitClassExtensions.find(this_ptr);
    unittypeext = UnitTypeClassExtensions.find(this_ptr->Class);

    if (unitext && unittypeext && unittypeext->WaterClass) {

        /**
         *  Store the original class so we can restore it when we are back on land.
         */
