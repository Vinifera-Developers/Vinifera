...
void Get_OS_Info(OSInfoStruct &os_info, uint32_t OSVersionPlatformId, uint32_t OSVersionNumberMajor, uint32_t OSVersionNumberMinor, uint32_t OSVersionBuildNumber)
{
    ...
    switch (OSVersionPlatformId) {
        ...
        default:
            std::memset(&os_info, 0, sizeof(os_info));
...
int32_t CPUDetectClass::ProcessorType = 0;
int32_t CPUDetectClass::ProcessorFamily = 0;
int32_t CPUDetectClass::ProcessorModel = 0;
int32_t CPUDetectClass::ProcessorRevision = 0;
int32_t CPUDetectClass::ProcessorSpeed = 0;

int64_t CPUDetectClass::ProcessorTicksPerSecond = 0;
...
uint32_t CPUDetectClass::FeatureBits = 0;
uint32_t CPUDetectClass::ExtendedFeatureBits = 0;

uint32_t CPUDetectClass::L3CacheSize = 0;
uint32_t CPUDetectClass::L3CacheLineSize = 0;
uint32_t CPUDetectClass::L3CacheSetAssociative = 0;
uint32_t CPUDetectClass::L2CacheSize = 0;
uint32_t CPUDetectClass::L2CacheLineSize = 0;
uint32_t CPUDetectClass::L2CacheSetAssociative = 0;
uint32_t CPUDetectClass::L1DataCacheSize = 0;
uint32_t CPUDetectClass::L1DataCacheLineSize = 0;
uint32_t CPUDetectClass::L1DataCacheSetAssociative = 0;
uint32_t CPUDetectClass::L1InstructionCacheSize = 0;
uint32_t CPUDetectClass::L1InstructionCacheLineSize = 0;
uint32_t CPUDetectClass::L1InstructionCacheSetAssociative = 0;
uint32_t CPUDetectClass::L1InstructionTraceCacheSize = 0;
uint32_t CPUDetectClass::L1InstructionTraceCacheSetAssociative = 0;

uint64_t CPUDetectClass::TotalPhysicalMemory = 0;
uint64_t CPUDetectClass::AvailablePhysicalMemory = 0;
uint64_t CPUDetectClass::TotalPageMemory = 0;
uint64_t CPUDetectClass::AvailablePageMemory = 0;
uint64_t CPUDetectClass::TotalVirtualMemory = 0;
uint64_t CPUDetectClass::AvailableVirtualMemory = 0;

uint32_t CPUDetectClass::OSVersionNumberMajor = 0;
uint32_t CPUDetectClass::OSVersionNumberMinor = 0;
uint32_t CPUDetectClass::OSVersionBuildNumber = 0;
uint32_t CPUDetectClass::OSVersionPlatformId = 0;
...
CPUDetectClass::IntelProcessorType CPUDetectClass::IntelProcessor = INTEL_PROCESSOR_UNKNOWN;
CPUDetectClass::AMDProcessorType CPUDetectClass::AMDProcessor = AMD_PROCESSOR_UNKNOWN;
CPUDetectClass::VIAProcessorType CPUDetectClass::VIAProcessor = VIA_PROCESSOR_UNKNOWN;
CPUDetectClass::RiseProcessorType CPUDetectClass::RiseProcessor = RISE_PROCESSOR_UNKNOWN;
...
static uint32_t Calculate_Processor_Speed(int64_t &ticks_per_second)
{
    ...
    uint32_t start = SystemTime.Get();
    uint32_t elapsed;

    while ((elapsed = SystemTime.Get() - start) < 200) {
        timer1 = __rdtsc();
    }
...
void CPUDetectClass::Init_Processor_String()
{
    ...
    if (ext_id.eax & 0x80000000) {
        ...
        char str[2048];
        std::strcat(str, Get_Processor_Manufacturer_Name());
...
#define CPU_LOG(n, ...) \
    do { \
        std::snprintf(work, sizeof(work), n, ##__VA_ARGS__); \
        std::strncat(ProcessorLog, work, sizeof(ProcessorLog)); \
    } while (false)
    ...
    if (CPUDetectClass::Get_L1_Instruction_Trace_Cache_Size() > 0) {
        CPU_LOG("L1 Instruction Trace Cache: %d way set associative, %dk\r\n",
            CPUDetectClass::Get_L1_Instruction_Cache_Set_Associative(),
            CPUDetectClass::Get_L1_Instruction_Cache_Size() / 1024);
    } else {
...
#define COMPACT_LOG(n, ...) \
    do { \
        std::snprintf(work, sizeof(work), n, ##__VA_ARGS__); \
        std::strncat(CompactLog, work, sizeof(CompactLog)); \
    } while (false)
...
static class CPUDetectInitClass
{
    public:
        CPUDetectInitClass()
        {
            ...
            /**
             *  The target games assume its going to run on x86 hardware, hopefully
             *  we will be able to change that eventually.
             */
            if (CPUDetectClass::Has_CPUID_Instruction()) {
